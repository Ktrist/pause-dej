...
-- =============================================================================
-- B2B TABLES - Admin Policies
-- =============================================================================

DROP POLICY IF EXISTS "Admins can view all b2b quotes" ON b2b_quotes;
DROP POLICY IF EXISTS "Admins can update b2b quotes" ON b2b_quotes;

CREATE POLICY "Admins can view all b2b quotes"
  ON b2b_quotes FOR SELECT
  USING (
    -- User can view their own quotes OR is admin
    auth.uid() = user_id
    OR EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
